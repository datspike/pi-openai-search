...
## Как сообщить агенту о поиске

`web_search` подключается к запросу провайдера, а не регистрируется как вызываемый Pi-инструмент. Поэтому агент, который ориентируется только на список Pi/MCP-инструментов, может ошибочно решить, что поиска нет. Например, в `~/.pi/agent/APPEND_SYSTEM.md` можно добавить такой текст:

> Если ответ зависит от актуальных внешних сведений или пользователь просит поискать в сети, сначала используй доступный в текущем модельном маршруте поиск. Нативный OpenAI `web_search`, подключённый через `pi-openai-search`, добавляется в запрос к провайдеру и не обязан отображаться среди вызываемых Pi- или MCP-инструментов. Отсутствие поискового инструмента в списке и отключённый MCP сами по себе не доказывают недоступность нативного поиска. Не приписывай поиск инструменту, вызова которого нет в наблюдаемых событиях.
>
> Считай поиск выполненным только при наличии наблюдаемых результатов и проверяемых источников. Учитывай уже полученные результаты: не повторяй поиск через другой инструмент только из-за различия его названия. Если запрос не дал результатов или маршрут поиска действительно недоступен, обозначь границу проверки; не подменяй его другим сервисом без разрешения пользователя.

Подсказка не гарантирует вызов: по умолчанию `tool_choice = "auto"`. Не считайте наличие инструмента в payload или `status: completed` доказательством найденного источника; проверяйте наблюдаемые события и источники.
